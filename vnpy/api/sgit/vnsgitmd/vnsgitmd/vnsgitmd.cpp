...
void MdApi::processRtnDeferDeliveryQuot(Task task) {
  PyLock lock;
  CThostDeferDeliveryQuot task_data =
      any_cast<CThostDeferDeliveryQuot>(task.task_data);
  dict data;
  data["InstrumentID"] = task_data.InstrumentID;
...
BOOST_PYTHON_MODULE(vnsgitmd) {
  ...
  class_<MdApiWrap, boost::noncopyable>("MdApi")
      ...
      .def("onRtnDeferDeliveryQuot",
           pure_virtual(&MdApiWrap::onRtnDeferDeliveryQuot));
};