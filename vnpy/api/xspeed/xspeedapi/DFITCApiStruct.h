...
struct APISTRUCT DFITCInsertOrderField {
  ...
  DFITCLocalOrderIDType
      localOrderID; //本地委托号, 由API使用者维护，在同一个会话中不能重复
  DFITCInstrumentIDType
      instrumentID; //合约代码,
                    //支持目前国内4个期货交易所的所有合约，包括大商所/郑商所的套利合约
  ...
  DFITCOrderTypeType orderType;   //报单类型, 支持限价
...
struct APISTRUCT DFITCMatchRtnField {
  ...
  DFITCAdjustmentInfoType
      adjustmentInfo; //组合或对锁的保证金调整信息,格式:[合约代码,买卖标志,投资类别,调整金额;]
  DFITCCustomCategoryType customCategory; //自定义类别
...
struct APISTRUCT DFITCEquityComputModeRtnField {
  DFITCCapControlModeType
      capConMode; //资金控制方式,该方式需要用户按位判断,可能为多种组合
