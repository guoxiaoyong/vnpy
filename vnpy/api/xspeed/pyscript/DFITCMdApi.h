...
public:
   ...
   * @param
   * pMarketDataField:指向行情信息结构的指针，结构体中包含具体的行情信息。
   */
  virtual void
  OnMarketData(struct DFITCDepthMarketDataField *pMarketDataField) {}
...
public:
   ...
   * @param
   * ppInstrumentID[]:指针数组，每个指针指向一个合约。（*代表订阅所有合约，也可订阅某交易所所有合约如DCE）
   * @param nCount:合约个数
   ...
   * @param
   * ppInstrumentID[]:指针数组，每个指针指向一个合约。（*代表订阅退订所有合约，也可退订某交易所所有合约如DCE）
   * @param nCount:合约个数
   * @return 0 - 请求发送成功; -1 - 请求发送失败
   */
   ...
   * 订阅询价
   * @param
   * ppInstrumentID[]:指针数组，每个指针指向一个合约。（*代表订阅所有询价，也可订阅某交易所所有询价如DCE）
   * @param nCount:合约个数
   * @return 0 - 请求发送成功; -1 - 请求发送失败
   */
   ...
   * 退订询价
   * @param
   * ppInstrumentID[]:指针数组，每个指针指向一个合约。（*代表订阅退订所有询价，也可退订某交易所所有询价如DCE）
   * @param nCount:合约个数
   * @return 0 - 请求发送成功; -1 - 请求发送失败
   */
   ...
   */
  virtual int
  ReqUserLogout(struct DFITCUserLogoutField *pReqUserLogoutField) = 0;
