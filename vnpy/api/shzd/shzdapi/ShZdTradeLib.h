...
public:
  ...
  virtual int RegisterFront(const char *pszFrontAddress,
                            int portNum) = 0; //连接前置服务器
  virtual int RegisterMarket(const char *pszMarketAddress,
                             int portNum) = 0; //连接行情服务器
  virtual int
  ShZdSendInfoToTrade(const CShZdMessage *send) = 0; //向前置服务器发送交易请求
  virtual int
  ShZdSendInfoToMarket(const CShZdMessage *send) = 0; //向行情服务器发送行情请求
