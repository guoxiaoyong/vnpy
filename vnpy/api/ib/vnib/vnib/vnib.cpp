...
void IbWrapper::tickPrice(TickerId tickerId, TickType field, double price,
                          int canAutoExecute) {
  PyLock lock;
...
void IbWrapper::tickOptionComputation(TickerId tickerId, TickType tickType,
                                      double impliedVol, double delta,
                                      double optPrice, double pvDividend,
                                      double gamma, double vega, double theta,
                                      double undPrice) {
  PyLock lock;
  this->api->tickOptionComputation(tickerId, tickType, impliedVol, delta,
                                   optPrice, pvDividend, gamma, vega, theta,
                                   undPrice);
};

void IbWrapper::tickGeneric(TickerId tickerId, TickType tickType,
                            double value) {
  PyLock lock;
  this->api->tickGeneric(tickerId, tickType, value);
};

void IbWrapper::tickString(TickerId tickerId, TickType tickType,
                           const std::string &value) {
  PyLock lock;
  this->api->tickString(tickerId, tickType, value);
};

void IbWrapper::tickEFP(TickerId tickerId, TickType tickType,
                        double basisPoints,
  ...
  PyLock lock;
  this->api->tickEFP(tickerId, tickType, basisPoints, formattedBasisPoints,
                     totalDividends, holdDays, futureLastTradeDate,
                     dividendImpact, dividendsToLastTradeDate);
};

void IbWrapper::orderStatus(OrderId orderId, const std::string &status,
                            double filled, double remaining,
                            double avgFillPrice, int permId, int parentId,
                            double lastFillPrice, int clientId,
                            const std::string &whyHeld) {
  PyLock lock;
  this->api->orderStatus(orderId, status, filled, remaining, avgFillPrice,
                         permId, parentId, lastFillPrice, clientId, whyHeld);
};

void IbWrapper::openOrder(OrderId orderId, const Contract &contract,
...
void IbWrapper::updatePortfolio(const Contract &contract, double position,
                                ...
                                const std::string &accountName) {
  PyLock lock;
  this->api->updatePortfolio(contract, position, marketPrice, marketValue,
...
void IbWrapper::execDetails(int reqId, const Contract &contract,
...
void IbWrapper::error(const int id, const int errorCode,
                      const std::string errorString) {
  PyLock lock;
  this->api->error(id, errorCode, errorString);
};

void IbWrapper::updateMktDepth(TickerId id, int position, int operation,
                               int side, double price, int size) {
  PyLock lock;
  this->api->updateMktDepth(id, position, operation, side, price, size);
};

void IbWrapper::updateMktDepthL2(TickerId id, int position,
                                 std::string marketMaker, int operation,
                                 int side, double price, int size) {
  PyLock lock;
  this->api->updateMktDepthL2(id, position, marketMaker, operation, side, price,
                              size);
};

void IbWrapper::updateNewsBulletin(int msgId, int msgType,
...
void IbWrapper::historicalData(TickerId reqId, const std::string &date,
                               double open, double high, double low,
                               double close, int volume, int barCount,
                               double WAP, int hasGaps) {
  PyLock lock;
  this->api->historicalData(reqId, date, open, high, low, close, volume,
...
void IbWrapper::scannerData(int reqId, int rank,
                            const ContractDetails &contractDetails,
  ...
  PyLock lock;
  this->api->scannerData(reqId, rank, contractDetails, distance, benchmark,
                         projection, legsStr);
};
...
void IbWrapper::realtimeBar(TickerId reqId, long time, double open, double high,
                            double low, double close, long volume, double wap,
                            int count) {
  PyLock lock;
  this->api->realtimeBar(reqId, time, open, high, low, close, volume, wap,
                         count);
};
...
void IbWrapper::position(const std::string &account, const Contract &contract,
...
void IbWrapper::accountSummary(int reqId, const std::string &account,
...
void IbWrapper::verifyAndAuthMessageAPI(const std::string &apiData,
...
void IbWrapper::positionMulti(int reqId, const std::string &account,
...
void IbWrapper::accountUpdateMulti(int reqId, const std::string &account,
                                   const std::string &modelCode,
                                   const std::string &key,
                                   const std::string &value,
                                   const std::string &currency) {
  PyLock lock;
  this->api->accountUpdateMulti(reqId, account, modelCode, key, value,
                                currency);
};
...
void IbWrapper::securityDefinitionOptionalParameter(
  ...
  std::vector<std::string> expirationsVector(expirations.begin(),
                                             expirations.end());
  std::vector<double> strikesVector(strikes.begin(), strikes.end());

  this->api->securityDefinitionOptionalParameter(
      reqId, exchange, underlyingConId, tradingClass, multiplier,
      expirationsVector, strikesVector);
};
...
void VnIbApi::eDisconnect() { this->client->eDisconnect(); };
...
void VnIbApi::reqMktData(TickerId id, const Contract &contract,
  ...
  this->client->reqMktData(id, contract, genericTicks, snapshot,
                           mktDataOptions);
};

void VnIbApi::cancelMktData(TickerId id) { this->client->cancelMktData(id); };

void VnIbApi::placeOrder(OrderId id, const Contract &contract,
                         const Order &order) {
  this->client->placeOrder(id, contract, order);
};

void VnIbApi::cancelOrder(OrderId id) { this->client->cancelOrder(id); };

void VnIbApi::reqOpenOrders() { this->client->reqOpenOrders(); };
...
void VnIbApi::reqIds(int numIds) { this->client->reqIds(numIds); };

void VnIbApi::reqContractDetails(int reqId, const Contract &contract) {
  this->client->reqContractDetails(reqId, contract);
};

void VnIbApi::reqMktDepth(TickerId tickerId, const Contract &contract,
                          int numRows,
...
void VnIbApi::cancelNewsBulletins() { this->client->cancelNewsBulletins(); };
...
void VnIbApi::reqAllOpenOrders() { this->client->reqAllOpenOrders(); };

void VnIbApi::reqManagedAccts() { this->client->reqManagedAccts(); };
...
void VnIbApi::reqRealTimeBars(TickerId id, const Contract &contract,
...
void VnIbApi::reqScannerParameters() { this->client->reqScannerParameters(); };

void VnIbApi::reqScannerSubscription(
...
void VnIbApi::reqCurrentTime() { this->client->reqCurrentTime(); };

void VnIbApi::reqFundamentalData(TickerId reqId, const Contract &contract,
...
void VnIbApi::calculateImpliedVolatility(TickerId reqId,
...
void VnIbApi::calculateOptionPrice(TickerId reqId, const Contract &contract,
...
void VnIbApi::reqGlobalCancel() { this->client->reqGlobalCancel(); };

void VnIbApi::reqMarketDataType(int marketDataType) {
  this->client->reqMarketDataType(marketDataType);
};

void VnIbApi::reqPositions() { this->client->reqPositions(); };

void VnIbApi::cancelPositions() { this->client->cancelPositions(); };

void VnIbApi::reqAccountSummary(int reqId, const std::string &groupName,
...
void VnIbApi::verifyRequest(const std::string &apiName,
...
void VnIbApi::verifyAndAuthRequest(const std::string &apiName,
                                   const std::string &apiVersion,
                                   const std::string &opaqueIsvKey) {
  this->client->verifyAndAuthRequest(apiName, apiVersion, opaqueIsvKey);
};

void VnIbApi::verifyAndAuthMessage(const std::string &apiData,
...
void VnIbApi::reqPositionsMulti(int reqId, const std::string &account,
                                const std::string &modelCode) {
...
void VnIbApi::reqAccountUpdatessMulti(int reqId, const std::string &account,
                                      const std::string &modelCode,
                                      bool ledgerAndNLV) {
  this->client->reqAccountUpdatessMulti(reqId, account, modelCode,
                                        ledgerAndNLV);
};
...
void VnIbApi::reqSecDefOptParams(int reqId, const std::string &underlyingSymbol,
...
struct IbApiWrap : VnIbApi, wrapper<VnIbApi> {
  ...
  virtual void tickOptionComputation(TickerId tickerId, TickType tickType,
                                     ...
                                     double undPrice) {
    try {
      this->get_override("tickOptionComputation")(
          tickerId, tickType, impliedVol, delta, optPrice, pvDividend, gamma,
          vega, theta, undPrice);
    } catch (error_already_set const &) {
      PyErr_Print();
    }
  ...
  virtual void tickEFP(TickerId tickerId, TickType tickType, double basisPoints,
    ...
    try {
      this->get_override("tickEFP")(tickerId, tickType, basisPoints,
                                    formattedBasisPoints, totalDividends,
                                    holdDays, futureLastTradeDate,
                                    dividendImpact, dividendsToLastTradeDate);
    } catch (error_already_set const &) {
      PyErr_Print();
    }
  ...
  virtual void orderStatus(OrderId orderId, const std::string &status,
    ...
    try {
      this->get_override("orderStatus")(orderId, status, filled, remaining,
  ...
  virtual void updatePortfolio(const Contract &contract, double position,
                               ...
                               const std::string &accountName) {
    try {
      this->get_override("updatePortfolio")(
  ...
  virtual void updateMktDepth(TickerId id, int position, int operation,
                              int side, double price, int size) {
    try {
      this->get_override("updateMktDepth")(id, position, operation, side, price,
                                           size);
    } catch (error_already_set const &) {
      PyErr_Print();
    }
  ...
  virtual void updateMktDepthL2(TickerId id, int position,
                                ...
                                int side, double price, int size) {
    try {
      this->get_override("updateMktDepthL2")(id, position, marketMaker,
  ...
  virtual void updateNewsBulletin(int msgId, int msgType,
                                  ...
                                  const std::string &originExch) {
    try {
      this->get_override("updateNewsBulletin")(msgId, msgType, newsMessage,
                                               originExch);
    } catch (error_already_set const &) {
      PyErr_Print();
    }
  ...
  virtual void historicalData(TickerId reqId, const std::string &date,
                              ...
                              double WAP, int hasGaps) {
    try {
      this->get_override("historicalData")(reqId, date, open, high, low, close,
  ...
  scannerData(int reqId, int rank, const ContractDetails &contractDetails,
    ...
    try {
      this->get_override("scannerData")(reqId, rank, contractDetails, distance,
  ...
  virtual void realtimeBar(TickerId reqId, long time, double open, double high,
                           ...
                           int count) {
    try {
      this->get_override("realtimeBar")(reqId, time, open, high, low, close,
                                        volume, wap, count);
    } catch (error_already_set const &) {
      PyErr_Print();
    }
  ...
  virtual void positionMulti(int reqId, const std::string &account,
                             ...
                             double avgCost) {
    try {
      this->get_override("positionMulti")(reqId, account, modelCode, contract,
                                          pos, avgCost);
    } catch (error_already_set const &) {
      PyErr_Print();
    }
  ...
  virtual void accountUpdateMulti(int reqId, const std::string &account,
                                  ...
                                  const std::string &currency) {
    try {
      this->get_override("accountUpdateMulti")(reqId, account, modelCode, key,
                                               value, currency);
    } catch (error_already_set const &) {
      PyErr_Print();
    }
  ...
  // std::string& exchange, int underlyingConId, const std::string& tradingClass,
  ...
  virtual void securityDefinitionOptionalParameter(
    ...
    try {
      this->get_override("securityDefinitionOptionalParameter")(
          reqId, exchange, underlyingConId, tradingClass, multiplier,
          expirations, strikes);
    } catch (error_already_set const &) {
      PyErr_Print();
...
namespace boost {
namespace python {
template <class T> struct pointee<ibapi::shared_ptr<T>> { typedef T type; };
} // namespace python
} // namespace boost
...
BOOST_PYTHON_MODULE(vnib) {
  ...
  class_<IbApiWrap, boost::noncopyable>("IbApi")
      ...
      .def("cancelCalculateImpliedVolatility",
           &IbApiWrap::cancelCalculateImpliedVolatility)
      ...
      .def("tickOptionComputation",
           pure_virtual(&IbApiWrap::tickOptionComputation))
      ...
      .def("deltaNeutralValidation",
           pure_virtual(&IbApiWrap::deltaNeutralValidation))
      ...
      .def("verifyAndAuthMessageAPI",
           pure_virtual(&IbApiWrap::verifyAndAuthMessageAPI))
      .def("verifyAndAuthCompleted",
           pure_virtual(&IbApiWrap::verifyAndAuthCompleted))
      ...
      .def("accountUpdateMultiEnd",
           pure_virtual(&IbApiWrap::accountUpdateMultiEnd))
      .def("securityDefinitionOptionalParameter",
           pure_virtual(&IbApiWrap::securityDefinitionOptionalParameter))
      .def("securityDefinitionOptionalParameterEnd",
           pure_virtual(&IbApiWrap::securityDefinitionOptionalParameterEnd))
  ...
  class_<CommissionReport>("CommissionReport")
      ...
      .def_readwrite("yieldRedemptionDate",
  ...
  class_<ScannerSubscription>("ScannerSubscription")
      ...
      .def_readwrite("maturityDateAbove",
                     &ScannerSubscription::maturityDateAbove)
      .def_readwrite("maturityDateBelow",
      ...
      .def_readwrite("excludeConvertible",
                     &ScannerSubscription::excludeConvertible)
      .def_readwrite("averageOptionVolumeAbove",
                     &ScannerSubscription::averageOptionVolumeAbove)
      .def_readwrite("scannerSettingPairs",
  ...
  class_<Contract>("Contract")
      ...
      .def_readwrite("lastTradeDateOrContractMonth",
  ...
  class_<Order>("Order")
      ...
      .def_readwrite("overridePercentageConstraints",
      ...
      .def_readwrite("deltaNeutralSettlingFirm",
                     &Order::deltaNeutralSettlingFirm)
      .def_readwrite("deltaNeutralClearingAccount",
                     &Order::deltaNeutralClearingAccount)
      .def_readwrite("deltaNeutralClearingIntent",
      ...
      .def_readwrite("deltaNeutralShortSaleSlot",
                     &Order::deltaNeutralShortSaleSlot)
      .def_readwrite("deltaNeutralDesignatedLocation",
      ...
      .def_readwrite("scalePriceAdjustInterval",
      ...
      .def_readwrite("isPeggedChangeAmountDecrease",
  ...
  class_<Order::OrderComboLegList, Order::OrderComboLegListSPtr>(
      "OrderComboLegList")
  ...
  class_<std::vector<ibapi::shared_ptr<OrderCondition>>>("OrderConditionList")
      .def(vector_indexing_suite<std::vector<ibapi::shared_ptr<OrderCondition>>,
                                 true>());
