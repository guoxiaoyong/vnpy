...
const char *EDecoder::processOpenOrderMsg(const char *ptr, const char *endPtr) {
  ...
  DECODE_FIELD(
      order.deltaNeutralOrderType); // ver 11 field (had a hack for ver 11)
  ...
  if (m_serverVersion >= MIN_SERVER_VER_PEGGED_TO_BENCHMARK) {
    ...
    if (conditionsSize > 0) {
      for (; conditionsSize; conditionsSize--) {
        ...
        ibapi::shared_ptr<OrderCondition> item =
            ibapi::shared_ptr<OrderCondition>(OrderCondition::create(
...
const char *EDecoder::processExecutionDataMsg(const char *ptr,
                                              const char *endPtr) {
  int version;
  DECODE_FIELD(version);
...
const char *EDecoder::processHistoricalDataMsg(const char *ptr,
  ...
  for (int ctr = 0; ctr < itemCount; ++ctr) {
    ...
    m_pEWrapper->historicalData(reqId, bar.date, bar.open, bar.high, bar.low,
                                bar.close, bar.volume, bar.barCount,
                                bar.average, bar.hasGaps == "true");
  }

  // send end of dataset marker
  std::string finishedStr =
      std::string("finished-") + startDateStr + "-" + endDateStr;
  m_pEWrapper->historicalData(reqId, finishedStr, -1, -1, -1, -1, -1, -1, -1,
                              0);
...
const char *EDecoder::processScannerDataMsg(const char *ptr,
  ...
  for (int ctr = 0; ctr < numberOfElements; ++ctr) {
    ...
    m_pEWrapper->scannerData(tickerId, data.rank, data.contract, data.distance,
                             data.benchmark, data.projection, data.legsStr);
  }
...
const char *EDecoder::processRealTimeBarsMsg(const char *ptr,
  ...
  m_pEWrapper->realtimeBar(reqId, time, open, high, low, close, volume, average,
                           count);
...
const char *EDecoder::processExecutionDataEndMsg(const char *ptr,
                                                 const char *endPtr) {
  int version;
  int reqId;
...
const char *
EDecoder::processSecurityDefinitionOptionalParameterMsg(const char *ptr,
                                                        const char *endPtr) {
  int reqId;
  std::string exchange;
  int underlyingConId;
  ...
  m_pEWrapper->securityDefinitionOptionalParameter(
      reqId, exchange, underlyingConId, tradingClass, multiplier, expirations,
      strikes);
...
const char *
EDecoder::processSecurityDefinitionOptionalParameterEndMsg(const char *ptr,
                                                           const char *endPtr) {
  int reqId;
...
bool EDecoder::DecodeField(int &intValue, const char *&ptr,
                           const char *endPtr) {
