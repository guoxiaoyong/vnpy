...
EReaderSSL::EReaderSSL(EClientSocketSSL *clientSocket, EReaderSignal *signal)
    : processMsgsDecoder_(clientSocket->EClient::serverVersion(),
      ...
      threadReadDecoder_(clientSocket->EClient::serverVersion(),
                         &defaultWrapper) {
  m_isAlive = true;
...
bool EReaderSSL::processNonBlockingSelect() {
  ...
  if (m_pClientSocket->fd() >= 0) {
    ...
    int ret = select(m_pClientSocket->fd() + 1, &readSet, &writeSet, &errorSet,
                     &tval);
...
void EReaderSSL::onSend() { m_pEReaderSignal->issueSignal(); }
...
void EReaderSSL::onReceive() {
  ...
  int nRes =
      m_pClientSocket->receive(m_buf.data() + nOffset, m_buf.size() - nOffset);
...
EMessage *EReaderSSL::readSingleMsg() {
  if (m_pClientSocket->usingV100Plus()) {
    ...
    while (msgSize == 0) {
      ...
      msgSize =
          EDecoder(m_pClientSocket->EClient::serverVersion(), &defaultWrapper)
              .parseAndProcessMsg(pBegin, pEnd);
    }
