...
const int MIN_SERVER_VER_SUPPORTED =
    38; // all supported server versions are defined in EDecoder.h

std::vector<EMutex> EClientSocketSSL::sslLocks(CRYPTO_num_locks());

void EClientSocketSSL::lockingFunc(int mode, int type, const char *file,
                                   int line) {
  if (mode & CRYPTO_LOCK)
    sslLocks[type].Enter();
  else
...
///////////////////////////////////////////////////////////
// member funcs
EClientSocketSSL::EClientSocketSSL(EWrapper *ptr, EReaderSignal *pSignal)
    : EClient(ptr, new ESocketSSL()) {
...
bool EClientSocketSSL::asyncEConnect() const { return m_asyncEConnect; }

void EClientSocketSSL::asyncEConnect(bool val) { m_asyncEConnect = val; }

bool EClientSocketSSL::eConnect(const char *host, unsigned int port,
                                int clientId, bool extraAuth) {
  if (m_fd == -2) {
...
void EClientSocketSSL::ImportRootCertificatesFromWindowsCertStore() {
  ...
  for (auto certCtx = CertEnumCertificatesInStore(store, 0); certCtx;
    ...
    auto cert =
        d2i_X509(0, (const unsigned char **)&pBuf, certCtx->cbCertEncoded);
...
bool EClientSocketSSL::eConnectImpl(int clientId, bool extraAuth,
  ...
  if (SSL_get_verify_result(m_pSSL) != X509_V_OK) {
    getWrapper()->error(NO_VALID_ID, SSL_FAIL.code(),
                        SSL_FAIL.msg() + "certificate verification failure");
...
bool EClientSocketSSL::isSocketOK() const { return (m_fd >= 0); }

int EClientSocketSSL::fd() const { return m_fd; }
...
void EClientSocketSSL::serverVersion(int version, const char *time) {
  ...
  if (usingV100Plus() ? (m_serverVersion < MIN_CLIENT_VER ||
...
bool EClientSocketSSL::handleSocketError(int res) {
  if (!handleSSLError(res)) {
    getWrapper()->error(NO_VALID_ID, SSL_FAIL.code(),
                        SSL_FAIL.msg() + ERR_error_string(res, 0));
...
void EClientSocketSSL::onError() { handleSocketError(); }
