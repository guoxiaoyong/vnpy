...
struct ComboLeg {
  ...
  bool operator==(const ComboLeg &other) const {
    return (conId == other.conId && ratio == other.ratio &&
            ...
            exemptCode == other.exemptCode && action == other.action &&
...
struct Contract {
  Contract()
      : conId(0), strike(0), includeExpired(false), comboLegs(NULL),
        underComp(NULL) {}
  ...
  std::string exchange;
  std::string primaryExchange; // pick an actual (ie non-aggregate) exchange
                               // that the contract trades on.  DO NOT SET TO
                               // SMART.
  std::string currency;
  ...
  // COMBOS
  std::string
      comboLegsDescrip; // received in open order 14 and up for all combos
...
struct ContractDetails {
  ContractDetails()
      : minTick(0), priceMagnifier(0), underConId(0), evMultiplier(0),
        callable(false), putable(false), coupon(0), convertible(false),
