...
///////////////////////////////////////////////////////////
// encoders
template <class T> void EClient::EncodeField(std::ostream &os, T value) {
  os << value << '\0';
}
...
template <> void EClient::EncodeField<bool>(std::ostream &os, bool boolValue) {
...
EClient::EClient(EWrapper *ptr, ETransport *pTransport)
    : m_pEWrapper(ptr), m_clientId(-1), m_connState(CS_DISCONNECTED),
      m_extraAuth(false), m_serverVersion(0), m_useV100Plus(true),
      m_transport(pTransport) {}

EClient::~EClient() {}

EClient::ConnState EClient::connState() const { return m_connState; }

bool EClient::isConnected() const { return m_connState == CS_CONNECTED; }

bool EClient::isConnecting() const { return m_connState == CS_CONNECTING; }
...
int EClient::serverVersion() { return m_serverVersion; }

std::string EClient::TwsConnectionTime() { return m_TwsTime; }
...
bool EClient::usingV100Plus() { return m_useV100Plus; }
...
void EClient::reqMktDepth(TickerId tickerId, const Contract &contract,
  ...
  if (m_serverVersion < MIN_SERVER_VER_TRADING_CLASS) {
    if (!contract.tradingClass.empty() || (contract.conId > 0)) {
      m_pEWrapper->error(tickerId, UPDATE_TWS.code(),
                         UPDATE_TWS.msg() + "  It does not support conId and "
                                            "tradingClass parameters in "
                                            "reqMktDepth.");
      return;
    }
  }
...
void EClient::reqScannerSubscription(
  ...
  if (m_serverVersion < MIN_SERVER_VER_TRADING_CLASS) {
    if (!contract.tradingClass.empty()) {
      m_pEWrapper->error(reqId, UPDATE_TWS.code(),
                         UPDATE_TWS.msg() + "  It does not support "
                                            "tradingClass parameter in "
...
void EClient::placeOrder(OrderId id, const Contract &contract,
  ...
  if (m_serverVersion < MIN_SERVER_VER_DELTA_NEUTRAL_CONID) {
    if (order.deltaNeutralConId > 0 ||
      ...
      m_pEWrapper->error(id, UPDATE_TWS.code(),
                         UPDATE_TWS.msg() + "  It does not support "
                                            "deltaNeutral parameters: ConId, "
                                            "SettlingFirm, ClearingAccount, "
                                            "ClearingIntent.");
      return;
    }
  }
...
void EClient::exerciseOptions(TickerId tickerId, const Contract &contract,
  ...
  if (m_serverVersion < MIN_SERVER_VER_TRADING_CLASS) {
    if (!contract.tradingClass.empty() || (contract.conId > 0)) {
      m_pEWrapper->error(tickerId, UPDATE_TWS.code(),
                         UPDATE_TWS.msg() + "  It does not support conId, "
                                            "multiplier and tradingClass "
                                            "parameters in exerciseOptions.");
      return;
    }
  }
...
void EClient::verifyRequest(const std::string &apiName,
  ...
  if (!m_extraAuth) {
    m_pEWrapper->error(NO_VALID_ID, UPDATE_TWS.code(),
                       UPDATE_TWS.msg() + "  Intent to authenticate needs to "
                                          "be expressed during initial connect "
                                          "request.");
    return;
  }
...
void EClient::reqAccountUpdatessMulti(int reqId, const std::string &account,
                                      const std::string &modelCode,
                                      bool ledgerAndNLV) {
  // not connected?
  if (!isConnected()) {
...
int EClient::processOnePrefixedMsg(const char *&beginPtr, const char *endPtr,
...
bool EClient::extraAuth() { return m_extraAuth; }
...
EWrapper *EClient::getWrapper() const { return m_pEWrapper; }

void EClient::setClientId(int clientId) { m_clientId = clientId; }

void EClient::setExtraAuth(bool extraAuth) { m_extraAuth = extraAuth; }

void EClient::setHost(const std::string &host) { m_host = host; }

void EClient::setPort(unsigned port) { m_port = port; }
...
bool EClient::isInBufferEmpty() const { return m_inBuffer.empty(); }
